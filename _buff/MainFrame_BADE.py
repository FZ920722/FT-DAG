...
def TempShape(_n, _max_s, _id, _od):
    assert _n >= _max_s
    if _n == _max_s:
        yield (_n, )
    else:
        __rn = _n - _max_s
        for __shx in NSG(__rn, (1, __rn), (1, _max_s), (0, int(pow(__rn, 2) / 4)), _id, _od):
...
def __DB_Data(_shash, _s:tuple, _id:int, _od:int, _jl:int, _hl:int, _md:int, _mu:int, _wd:int, _wu:int, _dt:str):
    ...
    if len(__db_data) > 0:
        ...
        __data_buff = []
        for __ri, __rd in enumerate(__GDGen):
            yield __rd
            __data_buff.append(NCDA(__rd))
        
        if len(__data_buff) > 0:
            TCURS.execute(f'''CREATE TABLE IF NOT EXISTS {__tname} (dag BLOB NOT NULL, c BLOB PRIMARY KEY,  w INTEGER NOT NULL,  
...
def NC(_s:tuple, _id:int, _od:int, _jl:int, _hl:int, _md:int, _mu:int, _wd:int, _wu:int, _ms:bool=True, _mc:bool=True):
    ...
    # if __l > 1:    # iod不能超过宽度；宽度可以大于iod
    #     __njl, __nhl = min(MaxJl(_s), _jl), max(MinHl(_s), _hl)
    #     __nid = min(_id, __nwu, (__n - _s[-1]) - (__l - 2) + 1) 
    #     __nod = min(_od, __nwu, (__n - _s[0]) - (__l - 2) + 1)
...
def SCSGen(_s:tuple, _id:int, _od:int, _jl:int, _hl:int, _md:int, _mu:int, _wd:int, _wu:int):
    ...
    if __l > 1 and _hl > 0 and _s[0] > 1 and _id > 0 and _od > 0 and _jl > 0:
        ...
        # if _s[-1] == 1:
        #     __ss = _s[:-1]
        #     __swd = max(max(__ss), _wd)
        # else:
        #     __ss = _s[:-1] + (_s[-1] - 1, )
        #     __swd = max(max(__ss), _wd - 1)

        # __sn, __shl = sum(__ss), min(_hl, __l - _jl)
        ...
        for __sscs in NC(__ss, _id, _od, _jl, __shl, __smd, __smu, __swd, _wu, _ms=False, _mc=True if _id > 1 else False):
            ...
            for __ps_dg in NC(__ps_s, _id, _od, _jl, __pl - _jl + 1, __npmd, __npmu,
                ...
                if __psd_sn <= _id:
                    ...
                    for __ls_dg in NC(__ls_s, _id, _od, _jl, max(1, _hl - __ls_ln), __nlmd, __nlmu, __nlwd, _wu, _ms=True, _mc=True):   # l-segment
                        if _wd <= max(__ls_dg.graph['w'], __ps_dg.graph['w']) <= _wu:
                ...
                for _jlx in range(1):
                    ...
                    for _hlx in range(1):   
                        ...
                        for _mux in range(1):
                            ...
                            # _mux = int(pow(__nnum, 2/3))
                            # _mux = int(pow(__nnum, 1/1))
                            # _mux = 30
                            ...
                            for _mdx in range(1):
                                ...
                                for _wux in range(1):
                                    ...
                                    for _wdx in range(1):
                                        # _wdx =  max(1, __nnum - 2)
                                        _wdx = 1
                                        # _wdx, _wux = 1, 4   # math.ceil(pow(__nnum, 1/3))
                                        # _idx, _odx = 1, 2
                                        ...
                                        # 大规模DAG生成实验 1：
                                        # _lx = 4 # __nnum - 2    # math.ceil(__nnum * 3/ 4)

                                        # _ldx, _lux = _lx, _lx
                                        # _max_s = __nnum - _lx
                                        # print(f"{_max_s}_{_lx}")

                                        ld, lu = max(_hlx,   1), min(__nnum - _wdx + 1, __nnum) 
                                        ...
                                        # _idx = 2  # _odx = 1  # _jlx = 4  # _hlx = 1  # _mdx = 0  # _mux = 6  # _wdx = 1  # _wux = 5
                                        # """
                                        s_num = 0
                                        # for __s in TempShape(__nnum, _max_s, _idx, _odx):
                                        for __s in NSG(__nnum, (ld, lu), (sd, su), (_mdx, _mux), _idx, _odx):
                                            s_num += 1
                                        # s_num = 0
                                        # for __s in NSG(__nnum, (ld, lu), (sd, su), (_mdx, _mux), _idx, _odx):
                                            # s_num += 1
                                        ...
                                        # print(f"算法生成_{__nnum}----{dag_num}--{et - st:.8f}----{(et - st) / max(1, dag_num):.6f}")
                                        print(f"{__nnum}_\t{s_num}_\t{dag_num}_\t{(et - st)  :.8f}")
                                        ...
                                        N_N = 0
                                        # TIMES2 = 0
                                        pd.DataFrame([{'nn':__nnum, 'dn':dag_num, 'ct': et - st}]).to_csv(f'id_{_idx}_od_{_odx}.csv', index=False,  header=False, mode='a')
                                        """ 
