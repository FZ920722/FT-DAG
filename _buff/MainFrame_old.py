...
def RG(_sg:tuple, _id:int, _od:int, _jl:int, _hl:int, _md:int, _mu:int, _wd:int, _wu:int):
    ...
    __nmd, __nmu, __nwd, __nwu = max(__sgmd, _md), min(__sgmu, _mu), max(__sgwd, _wd), min(__sgwu, _wu)    # __njl,  __nhl   = min(MaxJl(_s), _jl), max(MinHl(_s), _hl)    # iod不能超过宽度；宽度可以大于iod
