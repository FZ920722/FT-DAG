...
class FT_Dag_Generator:
    ...
    def __DB_Data(self, _hash_s, _s:tuple, _mr:Interval, _wr:Interval, _id:int, _od:int, _jld:int, _jlh:int, _sl:int, _dt:str):
        ...
        # C2 Data Not Existe
            ...
            TCURS.execute(f"INSERT INTO GTable (ts, m_l, m_u, w_l, w_u, id, od, jld, jlh, sl, L) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", \
            ...
            if len(__data_buff) > 0:
                TCURS.execute(f'''CREATE TABLE {__tname} ({', '.join([f'{_i} {_t[0]} {_t[1]}' for _i, _t in DTableHead.items()])});''')
            
                TCURS.executemany(f''' INSERT INTO {__tname} (c, m, w, id, od, jld, jlh,  sl, dag) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?) ''', 
                                [(__dx['c'], __dx['m'], __dx['w'], __dx['id'], __dx['od'], __dx['jld'], __dx['jlh'], __dx['sl'], __dx['dag'],) for __dx in __data_buff])
    ...
    """ (*) Main """
    def DG(self, _s:tuple, _mr:Interval, _wr:Interval, _id:int, _od:int, _jld:int, _jlh:int, _sl:int, _ms:bool=True, _mc:bool=True):
    ...
    def SCSGen(self, _s:tuple, _mr:Interval, _wr:Interval, _id:int, _od:int, _jld:int,  _jlh:int, _sl:int):
        ...
        if __n > 3 and __l > 1 and _s[0] > 1 and _sl > 0 and _id > 0 and _od > 0 and _jld > 0 and _jlh > 0:
            ...
            for __sd in self.DG(__ss, __smr, __swr, _id, _od, _jld, _jlh, min(_sl, __l - _jld), _ms=False, _mc=True if _id > 1 else False):
                ...
                if __cng:= self.CCD(__sd, __l, _id, _od, _jld, _jlh, _sl):
                    ...
                    for __pns in self.NPlGen(__cng, __l - 1, Interval(__ss_cn, _id) & (_mr - __ss_en)):
                        ...
                        if __pni_set != __sd_sink:
                            ...
                            if __scs_w in _wr:
                                __n_es = tuple([(__epi, __esi) for __epi, __esi in __sd.edges()]) + tuple([(__pi, __sn) for __pi in __pni_set])
                                ...
                                if __cx not in __cbuff:
                                            ...
                                            for __sinki in __scs.nodes():
                                                if __scs.nodes[__sinki]['h'] == 1:
                                                    if nx.has_path(__scs, source=__souri, target=__sinki):
                                                        __rlist.append(nx.shortest_path_length(__scs, source=__souri, target=__sinki) )
                                    ...
                                    __scs.graph = {'c': __cx, 's': _s, 'w':__scs_w, 'dt': 'SCS', 'm': len(__n_es),
                                                   ...
                                                   'sl': min(__rlist) + 1,
                                                   'jld': max([__scs.nodes[__esi]['d'] - __scs.nodes[__epi]['d'] for __epi, __esi in __scs.edges()]),
                                                   'jlh': max([__scs.nodes[__epi]['h'] - __scs.nodes[__esi]['h'] for __epi, __esi in __scs.edges()])}
                                    # DAG_Det(__scs)
                                    yield __scs
    ...
    def MCGen(self, _s:tuple, _mr:Interval, _wr:Interval, _id:int, _od:int, _jlh:int, _jld:int, _sl:int):
        ...
        if 1 < __l and 1 <= _id and 1 <= _od and  1 <= _jld:
            for __lgn in range(_s[0] if _sl > 1 else 1, _s[0] + 1):
                __ogn = _s[0] - __lgn
                for __tlg_d in self.number_combine(__lgn, (1 if _s[0] > __lgn else 2, min(_s[1], __lgn, min(_s[:_sl]))), (1, __lgn)):
                    ...
                    for __lg in self.LG(__tlg, _s[1:], _od, _sl):
                        ...
                        for __rdag_mc in self.RG(tuple(Counter(__mc_sg).items()), __mc_mr & _mr, __mc_wr & (_wr - __ogn), _id, _od, _jld, _jlh, _sl):
        ...
        # (2) Other Case;
            ...
            for __lx in range(1, __l): # ls层数：从小到大；
                ...
                if (1 < __p_s[0] or __p_l == 1) and (__s_s[0] <= _od) and (__p_s[-1] <= _id):
                    ...
                    __p_wr = Interval(__p_wr.lower if _wr.lower <= __s_wr.upper and __s_wr.lower <= _wr.upper else max(_wr.lower, __p_wr.lower), _wr.upper)
                    __p_mr = Interval(_mr.lower - __s_mr.upper - __p_wr.upper * __s_s[0], _mr.upper - __s_mr.lower - __p_s[-1] * __s_s[0])
                    # (2.1) P-Segment (pd are not MS-DAG)：lg的w可以达则上限不超标即可，否则pg必须达标；
                    for __ps_dg in self.DG(__p_s, __p_mr, __p_wr, _id, _od, _jld, _jlh, __p_l - _jld + 1, _ms=False, _mc=True if _id > 1 else False):
                        ...
                        if __psd_sn <= _id:
                            ...
                            for __ls_dg in self.DG(__s_s, __s_mr, __s_wr, _id, _od, _jld, _jld, max(1, _sl - __s_l), _ms=True, _mc=True):   # l-segment
    ...
    def FC(self, _pd, _sd):
        ...
        __rd_ns, __rd_es = list(),  [(__ex, __ey) for __ex, __ey in _pd.edges()] + [(__ex + __pd_n, __ey + __pd_n) for __ex, __ey in _sd.edges()] + list(product(__pd_sink, __sd_sour))
        ...
        __rd.graph |= {'dt': 'MS', 
                    ...
                    'jld': max(_pd.graph['jld'], _sd.graph['jld'], __pd_l + 1 - min(__rd.nodes[__pi]['d'] for __pi in __pd_sink)),
                    'jlh': max(_pd.graph['jlh'], _sd.graph['jlh'], __sd_l + 1 - min(__rd.nodes[__si]['h'] for __si in __sd_sour))}
        # DAG_Det(__rd)
        return __rd
...
if __name__ == "__main__":
    ...
    LocalPath = "E:\\workspace\\MyReserch\\DAG_Generator\\2025_FT-DAG\\data"
    ...
    TCURS.execute(f"CREATE TABLE IF NOT EXISTS GTable ({', '.join([f'{_i} {_t[0]} {_t[1]}' for _i, _t in GTableHead.items()])});")
